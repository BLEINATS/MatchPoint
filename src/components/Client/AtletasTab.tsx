  ...
  return (
    <div className="space-y-6">
      ...
      {atletas.length > 0 ? (
        <div 
          ref={scrollRef}
          className="flex overflow-x-auto gap-6 pb-4 -mx-6 px-6 no-scrollbar snap-x snap-mandatory md:grid md:grid-cols-2 lg:grid-cols-3 md:mx-0 md:px-0 md:overflow-visible"
        >
          {atletas.filter(p => p.status === 'disponivel').map(atleta => (
            <div key={atleta.id} className="w-11/12 flex-shrink-0 snap-center md:w-auto">
              <button 
                onClick={() => onViewProfile(atleta)}
                className="bg-white dark:bg-brand-gray-800 rounded-lg shadow-md p-4 border border-brand-gray-200 dark:border-brand-gray-700 flex flex-col text-left hover:shadow-lg hover:border-brand-blue-500 transition-all focus:outline-none focus:ring-2 focus:ring-brand-blue-500 h-full w-full"
              >
