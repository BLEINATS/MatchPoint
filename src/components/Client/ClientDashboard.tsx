...
import { Calendar, Compass, Search, CreditCard, LayoutDashboard, Loader2, CheckCircle, AlertCircle, ShoppingBag, Clock, Heart, DollarSign, Gift, Handshake, GraduationCap, Star, User, Users, Banknote, FileText, MessageSquare, Briefcase, Repeat, XCircle, LifeBuoy, Lock, Bell } from 'lucide-react';
import { isAfter, startOfDay, isSameDay, format, parse, getDay, addDays, isBefore, endOfDay, addMinutes, subDays, isWithinInterval, formatDistanceToNow, isPast, differenceInHours } from 'date-fns';
...
const ClientDashboard: React.FC = () => {
  ...
  const navItems = [
    ...
    { id: 'atleta_painel', label: 'Painel Atleta', icon: Star, visible: !!currentAtletaId },
    { id: 'professor_painel', label: 'Painel Professor', icon: Briefcase, visible: !!currentProfessorId },
  ...
  const renderContent = () => {
    ...
    switch (activeView) {
      ...
      case 'documentos': return <DocumentsTab />;
      case 'seguranca': return <div className="bg-white dark:bg-brand-gray-800 p-6 rounded-lg shadow-md"><SecurityTab /></div>;
      case 'notificacoes': return <div className="bg-white dark:bg-brand-gray-800 p-6 rounded-lg shadow-md"><NotificationSettingsTab profile={profileFormData} setProfile={setProfileFormData} /></div>;
      case 'suporte': return <div className="bg-white dark:bg-brand-gray-800 p-6 rounded-lg shadow-md"><SupportTab /></div>;
      default: return null;
    }
  };
  ...
  return (
    <>
      ...
      <AnimatePresence>{isHirePlayerModalOpen && reservationToHireFor && (<HirePlayerModal isOpen={isHirePlayerModalOpen} onClose={() => setIsHirePlayerModalOpen(false)} onConfirm={(profId) => handleHireProfessional(reservationToHireFor.id, profId)} reserva={reservationToHireFor} profissionais={atletas} />)}</AnimatePresence>
