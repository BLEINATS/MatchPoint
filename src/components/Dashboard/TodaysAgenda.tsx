...
const TodaysAgenda: React.FC<TodaysAgendaProps> = ({ reservations = [], quadras, arenaName, className }) => {
  return (
    ...
    >
      ...
      <div className="space-y-2 flex-grow overflow-y-auto px-2 pb-2 -mx-2 min-h-0">
        {reservations.length > 0 ? reservations.map(r => {
          ...
          return (
            <div key={r.id} className="flex items-start gap-3 p-2 bg-brand-gray-50 dark:bg-brand-gray-700/50 rounded-lg">
              <div className="flex flex-col items-center justify-center rounded-lg p-2 w-16 text-center bg-brand-blue-500 text-white flex-shrink-0">
                <span className="font-bold text-lg">{r.start_time.slice(0, 5)}</span>
                <span className="text-xs">às {r.end_time.slice(0, 5)}</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-brand-gray-800 dark:text-brand-gray-200 truncate pr-2">{r.clientName}</p>
