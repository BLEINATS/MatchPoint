...
const Reservations: React.FC = () => {
  ...
  useEffect(() => {
    loadData();
    // Adiciona um listener para recarregar os dados quando a janela/aba ganha foco
  ...
  const displayedReservations = useMemo(() => {
    ...
    if (viewMode === 'list') { 
        viewStartDate = startOfDay(new Date(2020, 0, 1)); 
        viewEndDate = endOfDay(addYears(new Date(), 5)); 
    }
  ...
  const handleClearFilters = () => { setFilters({ status: 'all', type: 'all', clientName: '', quadraId: 'all' }); setIsFilterPanelOpen(false); };
  ...
  const handleSaveReservation = async (reservaData: Omit<Reserva, 'id' | 'created_at'> | Reserva) => {
    ...
    try {
      ...
      const dataToUpsert: Partial<Reserva> = { ...reservaData, arena_id: arena.id, profile_id: alunoForReservation?.profile_id || null };
      if (!isEditing) dataToUpsert.created_by_name = profile.name;
